create table if not exists public.challenge_pool_user (
    ...
    challenge_pool_owner uuid references auth.users on delete cascade not null,
...
CREATE POLICY "cpu_select_policy_for_user" ON public.challenge_pool_user FOR SELECT USING ((auth.uid() = user_id) or (auth.uid() = challenge_pool_owner));