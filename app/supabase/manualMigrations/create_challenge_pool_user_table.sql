...
CREATE POLICY "cpu_select_policy_for_cp_owner" ON public.challenge_pool_user FOR SELECT USING (auth.uid() IN ( SELECT challenge_pools.owner FROM challenge_pools WHERE (challenge_pool_user.challenge_pool = challenge_pools.id)));