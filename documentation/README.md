...
## Vision
Lightbulb Learning hat die Vision, auf das langfristige Lernen zu optimieren. Die Anforderungen an Absolventen der Zukunft sind nicht die gleichen wie die, der Vergangenheit. Sie müssen kommunizieren, kollaborieren, komplexe Problemstellungen analysieren und Lösungen als Team entwickeln können. Aus dem Zugang zum Internet, und dem Wissen um den richtigen Umgang mit der gigantischen Menge an Informationen erwächst die Fähigkeit, die Lösungen für die Probleme der Zukunft entwickeln zu können.
...
Scala kombiniert objektorientierte und funktionale Programmierung in einer Sprache. Durch statische Typen können Fehler in komplexen Anwendungen vermieden werden. Scala verfügt über viele Module und sogar Java Libraries können direkt in Scala eingesetzt werden. Der Code kann in Bytecode für die JVM kompiliert werden. Dadurch muss keine neue Laufzeitumgebung installiert werden. Scala 3 hat viele Breaking changes, sodass es nicht mühelos ist, Code für Scala 2 auszuführen.
...
Die Scala Dateien wurden dann in eine FAT-JAR Datei kompiliert und als ZIP-Datei implizit über das AWS-CDK bzw. CloudFormation hochgeladen.
...
Dabei handelt es sich um Node Package Module (NPM), welche auch über das AWS-CDK bzw. Cloud Formation hochgeladen werden.
...
### Cognito
[Amazon Cognito](https://aws.amazon.com/cognito/) erlaubt es schnell und einfach Benutzeranmeldungen und Zugriffskontrollen für Web- und Mobil Anwendungen hinzuzufügen. Über Userpools lassen sich beispielsweise pro Premium Tenant mehrere Benutzer abbilden. Zudem ist es möglich innerhalb eines Userpools Benutzer einer oder mehreren Gruppen hinzuzufügen. In Cognito lassen sich auch die Attribute eines jeden Nutzers definieren. Bei Lightbulb Learning setzen wir beispielsweise ein eigenes Attribut: `admin_of_group`. Damit lässt sich dann bestimmen welcher User der Administrator einer Gruppe ist.

### DynamoDB
[Amazon DynamoDB](https://aws.amazon.com/dynamodb/) ist eine verwaltete NoSQL Key-Value Datenbank. DynamoDB bietet integrierte Sicherheit, kontinuerliche Backups, Replikationen basierend auf Regionen und Datenbankverwaltungswerkzeuge. In diesem Projekt wird die DynamoDB mit AppSync eingesetzt. Für jeden premium Tenant werden separate DynamoDB Tabellen provisioniert. 
...
### Microservices
Es wurde ein Microservice mit `Docker` erstellt. Dieser Dockercontainer konnte dann bei der [Amazon Elastic Container Registry (Amazon ECR)](https://aws.amazon.com/de/ecr/) gespeichert werden. Dadurch war man in der Lage einen [Amazon Elastic Kubernetes Service (EKS)](https://aws.amazon.com/de/eks/) einzurichten, welcher auf den gespeicherten Dockercontainer zugreifen kann. Das EKS wurde so eingerichtet, dass ein Kubernetes Cluster mit zwei EC2 Instanzen ausgeführt wird.

Nachdem die meiste Funktionalität mit AWS Lambdas umgesetzt wurde, haben wir entschieden, dass kein Microservice für das Projekt benötigt wird. Der größte Vorteil, der aus dieser Entscheidung entsteht, ist, dass durch die Lambdas um ein vielfaches weniger Kosten entstehen. Die EC2 Instanzen müssten durchgehend aktiv sein, während die Lambdas nur für die eigentliche Ausführungszeit berechnet werden.
...
Da die Zeitpunkte und der Umfang der auftretenden Ereignisse von äußeren Faktoren wie Nutzereingaben abhängt, muss ein Weg definiert sein, um auch mit starken Schwankungen in der Lastmenge umgehen zu können. Dabei unterstützt der [Simple Queue Service (SQS)](https://aws.amazon.com/de/sqs/) der AWS. Die Queues werden als Abonennten der [Simple Notification Service (SNS)](https://aws.amazon.com/de/sns/) Topics definiert, und halten die Events persistent und in der richtigen Reihenfolge, bis sie von den zuständigen Abonennten abgearbeitet werden können.
...
Bei der Umsetzung dieser Architektur bemerkten wir die damit zusammenhängenden Hürden: in erster Linie in der Hinsicht der zur Verfügungstellung der Daten und Resultate von Operationen aus Nutzersicht. Dieser ist nicht bereit (und sein Browser nicht darauf ausgelegt), nach einer Interaktion länger als ein paar hundert Millisekunden auf ein Ergebnis zu warten. Eine so schnelle Antwort ist bei schreibenden Operationen mit JVM basierten Lambdas (wie unsere Scala 3 Lambdas) völlig unrealistisch, da die Anfragen im Falle von cold-starts leicht über 10 Sekunden dauern können. Desweiteren steigt die Komplexität der Applikation durch die breitere Verteilung der Verantwortung, und die dadurch entstehende Vielzahl an internen Schnittstellen noch weiter. Somit entschlossen wir, die Topics mit den Events zwar zu behalten, und somit die Erweiterbarkeit um unabhängige Microservices zu gewährleisten, diese jedoch nicht im Rahmen des Projekts in Cloud Application Development zu implementieren.
...
Dabei erstellt man sogenannte Konstrukte für die jeweiligen Cloud Ressourcen. Die Konstrukte haben Voreinstellungen, will man von diesen abweichen, so kann man dies beispielsweise über gewisse Parameter konfigurieren. Auch das Schreiben von eigenen Funktionen für die Erstellung bestimmter Konstrukte haben wir erfolgreich eingesetzt. Beispielsweise für das Provisionieren der Lambda-Funktionen. Das AWS-CDK provisioniert dann die Resourcen mittels AWS CloudFormation. Bei Lightbulb Learning haben wir für das CDK anfangs Java eingesetzt. Später haben wir uns für TypeScript entschieden, da hier die Dokumentation aktueller war. 
